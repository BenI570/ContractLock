...
/// @notice Multiple escrows per user; supports native token or ERC-20 tokens.
contract MultiTokenEscrow is ReentrancyGuard {
    struct Escrow {
        address creator;
        address beneficiary;
        uint256 deadline;
    ...
    mapping(address => uint256[]) private userEscrows; // track escrows per user
    ...
    event RefundWithdrawn(uint256 indexed escrowId, address indexed payer, uint256 amount);
    event BeneficiaryClaimed(uint256 indexed escrowId, address indexed beneficiary, uint256 amount);
    ...
    function createEscrow(
        ...
        address _tokenAddress // address(0) for native token
        ...
        for (uint i = 0; i < _payers.length; i++) {
            ...
            e.isPayer[p] = true;
            e.payers.push(p);
            userEscrows[p].push(escrowId); // track escrows for each payer
        }

        userEscrows[msg.sender].push(escrowId); // track escrows for creator

        emit EscrowCreated(escrowId, msg.sender, _beneficiary, _amountPerPayer, _deadlineUnix, _payers, _tokenAddress);
    }

    /// @notice payer deposits into escrow
    ...
    /// @notice refund after deadline if not all paid
    ...
    function getEscrowPayers(uint256 escrowId) external view returns (address[] memory) {
        return escrows[escrowId].payers;
    }

    function depositedOf(uint256 escrowId, address payer) external view returns (uint256) {
        return escrows[escrowId].deposited[payer];
    }

    function getEscrowToken(uint256 escrowId) external view returns (address) {
